...
import com.ckey.shijing.exception.InterfaceChangedNotUsedException;
...
import com.ckey.shijing.exception.SessionInvalidateException;
...
public class BaseController { // 异常处理的类
	...
	public Result exceptionHandler(Exception ex, HttpServletResponse response,
		...
		if (ex.getClass() == NoSuchRequestHandlingMethodException.class) {
			...
			result.msg = "此接口暂停使用";
		} else if (ex.getClass() == MissPassException.class){
			result.error_code = "1001";
			result.msg = "缺少密码";
		}else if (ex.getClass() == MissUserNameException.class){
